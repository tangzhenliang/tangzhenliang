...
pages/
--| goods/ 二级
-----| index.vue 默认页 可以整个文件留空，goods就无默认页，一级的goods.vue替换这里的index会二级展示区替换一级展示区
...
login： this.$cookies.set('user',res.data) //种token到浏览器的cookie里

axios设置

```js
$axios.onRequest(config => {
    console.log('拦截器onRequest',app.$cookies.get('user'))//nuxt.config配置ssr后，第一次服务端运行，之后的路由跳转在客户端运行
    config.headers.token = app.$cookies.get('user') ? app.$cookies.get('user').token : ''
    return config;
  })

  // 返回回调
  $axios.onResponse(res => {
    // console.log('axios onResponse', res.data)
    if(res.data.err==2 && route.fullPath!=='/login'){
...
优化：登录时，存vuex和cookie，nuxtServerInit取出cookies，存vuex，axios拦截找vuex拿，不强刷，不读本地
...
它允许您永久保持应用**程序活跃**，无需停机即可重新加载它们，并不需要传统部署的.nuxt文件夹，该部署方法也跟生产环境一样含热更新
